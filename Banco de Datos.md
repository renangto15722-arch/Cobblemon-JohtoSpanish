ITENS E TERMOS A TRADUZIR
-------------------------------------------------------------------------
[Poké Balls y categoría]
- Balls =
- Dusk Ball =
- Fast Ball = Rapid Ball
- Friend Ball = Amigo Ball
- Great Ball = Super Ball
- Heal Ball = Sana Ball
- Heavy Ball = Peso Ball
- Level Ball = Nivel Ball
- Love Ball = Amor Ball
- Lure Ball = Cebo Ball
- Master Ball = Master Ball
- Moon Ball = Luna Ball
- Nest Ball = Nido Ball
- Net Ball = Malla Ball
- Poke Ball = Poké Ball
- Poké Ball = Poké Ball
- Poké Balls = Poké Balls
- Premier Ball = Honor Ball
- Quick Ball = Veloz Ball
- Safari Ball = Safari Ball
- Ultra Ball = Ultra Ball

- [Vitaminas / combate]
- Calcium = 
- Carbos = 
- Dire Hit = 
- Guard Spec = 
- HP Up = 
- Iron = 
- Protein = 
- X Accuracy = 
- X Attack = 
- X Defend = 
- X Special = 
- X Special Attack = 
- X Special Defence = 
- X Speed = 
- Zinc = 

[Piedras evolutivas]
- Dawn Stone = Piedra Alba
- Dusk Stone = Piedra Noche
- Everstone = Piedra Eterna
- Fire Stone = Piedra Fuego
- Leaf Stone = Piedra Hoja
- Moon Stone = Piedra Lunar
- Shiny Stone = Piedra Día
- Sun Stone = Piedra Solar
- Thunder Stone = Piedra Trueno
- Water Stone = Piedra Agua

[Objetos evolutivos / equipables]
- Black Belt = Cinturón Negro
- Bright Powder = Polbo Brillo
- Choice Band = Cinta Elección
- Choice Scarf = Pañuelo Elección
- Dragon Scale = Escama Dragón
- Dubious Disc = Disco Extraño
- Electirizer = Electrizador
- Flame Orb = Llamasfera
- Focus Band = Cinta Aguante
- Focus Sash = Banda Aguante
- King's Rock = Roca del Rey
- Link Cable = Cable Link
- Magmarizer = Magmatizador
- Metal Coat = Revestimiento Metálico
- Metronome = Metrónomo
- Muscle Band = Cinta Fuerte
- Power Anklet = Franja Recia
- Power Band = Banda Recia
- Power Belt = Cinturón Recio
- Power Bracer = Brazal Recio
- Power Herb = Hierba Única
- Power Lens = Lente Recia
- Power Weight = Pesa Recia
- Protector = Protector
- Razor Claw = Garra Afilada
- Razor Fang = Colmillo Agudo
- Scope Lens = Periscopio
- Silk Scarf = Pañuelo Seda
- Soft Sand = Arena Fina
- Toxic Orb = Toxisfera
- TwistedSpoon = Cuchara Torcida
- Up-Grade = Mejora
- White Herb = Hierba Blanca
- Wide Lens = Lupa
- Zoom Lens = Telescopio

[Bayas]
Obs.: Los nombres de las Bayas se mantienen en el original (sin traducción definida).
- Qualot Berry =
- Tamato Berry =
- Wacan Berry =
- Chople Berry =
- Coba Berry =
- Pomeg Berry =
- Hondew Berry =
- Occa Berry =
- Rindo Berry =
- Kebia Berry =
- Payapa Berry =

[Cañas de pescar / bicicleta]
- Old Rod = 
- Good Rod = 
- Super Rod = 
- Bicycle / Bike -> Puede traducirse como bicicleta.

[Monedas / premios]
- Coins = Monedas

-------------------------------------------------------------------------
GOLPES / HMs / TMs
-------------------------------------------------------------------------
- Charge Beam -> mantener el nombre original cuando sea un movimiento. =
- Cut HM -> mantener el nombre original como objeto/técnica. =
- Flash HM -> mantener el nombre original como objeto/técnica. =
- HM -> mantener la sigla original. =
- Rock Smash -> mantener el nombre original cuando sea un objeto/etiqueta/comando; en diálogos puede traducirse como "Golpe Roca". =
- Strength -> mantener el nombre original cuando sea un movimiento/objeto. =
- Surf -> mantener el nombre original cuando sea un movimiento/objeto. =
- TM -> mantener la sigla original. =
- TM57 -> mantener el nombre original. =
- Whirlpool -> mantener el nombre original cuando se reciba como movimiento u objeto. =
-------------------------------------------------------------------------
LUGARES - JOHTO
-------------------------------------------------------------------------
- New Bark Town -> Pueblo Primavera
- Cherrygrove City -> Ciudad Cerez
- Violet City -> Ciudad Malva
- Azalea Town -> Pueblo Azalea
- Goldenrod City -> Ciudad Trigal
- Ecruteak City -> Ciudad Iris
- Olivine City -> Ciudad Olivo
- Cianwood City -> Ciudad Orquídea
- Mahogany Town -> Pueblo Caoba
- Blackthorn City -> Ciudad Endrino
- Lake of Rage -> Lago de la Furia
- Ilex Forest -> Bosque Ilex
- National Park -> Parque Nacional
- Ruins of Alph -> Ruinas Alfa
- Sprout Tower -> Torre Bellsprout
- Slowpoke Well -> Pozo Slowpoke
- Mt. Mortar -> Monte Mortero
- Mt. Silver -> Monte Plateado
- Mt. Moon -> Monte Luna
- Dragon's Den -> Guarida Dragón
- Union Cave -> Cueva Unión
- Ice Path -> Ruta Helada
- Bell Tower -> Torre Campana
- Brass Tower -> Torre Latón
- Tin Tower -> Torre Hojalata
- Burned Tower -> Torre Quemada
- Whirl Islands -> Islas Remolino
- Route -> Ruta
- Cycling Road -> Ruta Ciclista
- Dark Cave -> Cueva Oscura

-------------------------------------------------------------------------
LUGARES - KANTO
-------------------------------------------------------------------------
- Pallet Town -> Pueblo Paleta
- Viridian City -> Ciudad Verde
- Pewter City -> Ciudad Plateada
- Cerulean City -> Ciudad Celeste
- Vermilion City -> Ciudad Carmín
- Lavender Town -> Pueblo Lavanda
- Celadon City -> Ciudad Azulona
- Fuchsia City -> Ciudad Fucsia
- Saffron City -> Ciudad Azafrán
- Saffron (suelto, referido a la ciudad) -> Ciudad Azafrán
- Cinnabar Island -> Isla Canela
- Indigo Plateau -> Meseta Añil
- Viridian Forest -> Bosque Verde
- Diglett's Cave -> Cueva Diglett
- Seafoam Islands -> Islas Espuma
- Power Plant -> Central Eléctrica
- Pokémon Mansion -> Mansión Pokémon
- Safari Zone -> Zona Safari
- Victory Road -> Calle Victoria
- Rock Tunnel -> Túnel Roca
- Fighting Dojo -> Dojo Karate

-------------------------------------------------------------------------
INSTALACIONES / OTROS LUGARES
-------------------------------------------------------------------------
- Battle Frontier -> Frente Batalla
- Battle Tower -> Torre Batalla
- Exchange Service Corner -> Servicio de Canje
- Day Care / Day-care -> Guardería
- Department Store -> Centro Comercial
- Game Corner -> Game Corner
- Gym -> Gimnasio
- Gym Leader -> Líder de Gimnasio
- Lab / Laboratory -> Laboratorio
- Lighthouse -> Faro
- Pokémon Center -> Centro Pokémon
- Poké Mart / Mart -> Tienda Pokémon
- Radio Tower -> Torre Radio
- S.S. Anne -> S.S. Anne
- S.S. Aqua -> S.S. Aqua
- Trainer House -> Casa Entrenador
- Silph Co's Head Office -> Sede Central de Silph S.A.
- Silph Co -> Silph S.A.
- Magnet Train Station -> Estación del Magnetotrén
- Magnet Train (el tren/transporte) -> Magnetotrén
  Obs.: el ITEM "Magnet Train Pass" permanece en original (ver seccion de items).

-------------------------------------------------------------------------
NOMBRES / GRUPOS
-------------------------------------------------------------------------
- Team Rocket -> Team Rocket
- Rocket -> Rocket
- Rockets -> Rockets
- Kimono Girls -> Kimono Girls
- Elite Four -> Alto Mando
- Pokémon League -> Liga Pokémon
- Champion -> Campeón
- Pokémon Master -> Maestro Pokémon
